...
then
	echo ""
	echo "The TCP Tweaker network settings have already been added to the system!"
	echo ""
	read -p "Do you want to remove the TCP Tweaker settings? [s/n]: " -e -i n answer0
	if [[ "$answer0" = 's' ]]; then
		grep -v "^#PH56
...
sysctl -p /etc/sysctl.conf > /dev/null
		echo ""
		echo "The TCP Tweaker network settings have been successfully removed."
		echo ""
	exit
	else 
		echo ""
